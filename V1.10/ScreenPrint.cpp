...
/*                                                                            */
/*----------------------------------------------------------------------------*/
#define SPVersion 1.10
#define FrameWorkVersion 3.00
/*————————————————————全局初始化————————————————————*/
//--引用函数库
#include "vex.h"
#include "Control.h"
...
int MainPrint(void)
{
  ...
  while(1)
  {
    ...
    else if(Page == 1)
    {
      ...
      LinePrint(6,5,1,"Absolute Pointing System Version",APSVersion);
      LinePrint(7,5,1,"Std Framework Version",FrameWorkVersion);
      if(Brain.Screen.pressing() == 1 && Brain.Screen.xPosition() > 440 && Brain.Timer.value() - PressTime > 1)
      {
        Brain.Screen.clearScreen();
        PressTime = Brain.Timer.value();
