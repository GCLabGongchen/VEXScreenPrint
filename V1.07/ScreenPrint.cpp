...
int MainPrint(void)
{
  ...
  while(1)
  {
    ...
    if(Page == 0)
    {
      ...
      //下限检测
      if(RowChange + RowLimit < 12 && (TouchControl < 0 || Port21.position(degrees) < 0))
      {
    ...
    else if(Page == 1)
    {
      ...
      LinePrint(2,1,0,"Author : GongChen & ChenEnXiu");
      LinePrint(4,1,1,"Version",SPVersion);
      if(Brain.Screen.pressing() == 1 && Brain.Screen.xPosition() > 440 && Brain.Timer.value() - PressTime > 1)
      {
        Brain.Screen.clearScreen();
        PressTime = Brain.Timer.value();
